...
static inline int mmio_is_mapped(uint32_t addr)
{
    int err = -1;
    int i = 0;
...
static inline uint32_t mmio_get(uint32_t addr, int res)
{
...
static inline void mmio_set(uint32_t addr, uint32_t val, int res)
{
    int i = mmio_is_mapped(addr);
